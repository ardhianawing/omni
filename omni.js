#!/usr/bin/env node
// ═══════════════════════════════════════════════════════════
//  Omni — Multi-Provider AI Coding Assistant
//  One CLI, any LLM provider
// ═══════════════════════════════════════════════════════════
...
const DS_DIR = path.join(HOME, ".omni");
...
const READONLY_MODE = process.env.OMNI_READONLY === "1";
...
const PROVIDERS = {
  deepseek: {
    ...
    pricing: { "deepseek-chat": { input: 0.14, output: 0.28 }, "deepseek-reasoner": { input: 0.55, output: 2.19 } },
  ...
  openrouter: {
    ...
    extraHeaders: { "HTTP-Referer": "https://github.com/omni-cli" },
  },
};
...
function detectProvider(modelName) {
  for (const [id, p] of Object.entries(PROVIDERS)) {
    if (p.models.includes(modelName)) return id;
  }
...
function getProvider() {
  return PROVIDERS[detectProvider(config.model)];
}
...
function getApiKey() {
  ...
  if (!key) {
    console.error(C.red(`\n  Error: ${provider.keyEnv} not set.`));
    console.error(C.dim(`  Run: export ${provider.keyEnv}=your_api_key`));
    return null;
  }
  return key;
}
...
function getModelPricing() {
  const provider = getProvider();
  return provider.pricing[config.model] || { input: 0, output: 0 };
}
...
function isSafePath(filePath) {
  ...
  if (absPath === HOME || absPath.startsWith(HOME + '/.')) {
    // Allow .omni folder operations (for config/sessions)
    if (absPath.startsWith(path.join(HOME, '.omni'))) {
      return true;
    }
    return false;
  }
...
const SYSTEM_PROMPT = `You are Omni, an interactive AI coding assistant running in the user's terminal.
...
function isReasoningModel(model) {
  return model === "deepseek-reasoner" || model === "o1" || model === "o1-mini" || model.startsWith("o3");
}
...
function streamChat() {
  return new Promise((resolve, reject) => {
    ...
    if (!apiKey) return reject(new Error("API key not configured"));

    const useTools = !isReasoningModel(config.model);
    const body = {
      model: config.model,
      messages,
      stream: true,
    };
    // Some providers (Groq) may not support stream_options
    try { body.stream_options = { include_usage: true }; } catch {}
...
- **/provider [name]** — Show/switch provider (deepseek, openai, groq, openrouter)
- **/providers** — List all providers and API key status
...
Switch between providers by setting a model name:
...
`));
    ...
    case "/model":
      if (!arg1) {
        ...
        console.log(`  Current: ${C.bold(curProvider.name + ":" + config.model)}`);
        console.log();
        for (const [id, p] of Object.entries(PROVIDERS)) {
          const isCur = detectProvider(config.model) === id;
          const tag = isCur ? C.green(" (active)") : "";
          const keyStatus = process.env[p.keyEnv] ? C.green("✓ KEY") : C.red("✗ " + p.keyEnv);
          console.log(`  ${C.bold(p.name)}${tag}  ${keyStatus}`);
          if (p.models.length > 0) {
            for (const m of p.models) {
              const cur = m === config.model ? C.green(" ← current") : "";
              console.log(`    ${C.dim("·")} ${C.cyan(m)}${cur}`);
            }
          } else {
            console.log(`    ${C.dim("· (accepts any model string, e.g. provider/model)")}`);
          }
        }
        console.log();
      } else {
        config.model = arg1;
        const detected = detectProvider(arg1);
        const prov = PROVIDERS[detected];
        saveConfig(config);
        console.log(`  Model: ${C.bold(arg1)}  Provider: ${C.cyan(prov.name)}`);
        if (isReasoningModel(arg1)) console.log(C.dim("  Reasoning model — tools disabled."));
        const keyOk = process.env[prov.keyEnv];
        if (!keyOk) console.log(C.yellow(`  Warning: ${prov.keyEnv} not set.`));
      }
      return true;
    ...
    case "/providers":
    case "/provider": {
      if (!arg1 || cmd === "/providers") {
        console.log(C.bold("\n  Providers:"));
        for (const [id, p] of Object.entries(PROVIDERS)) {
          const isCur = detectProvider(config.model) === id;
          const tag = isCur ? C.green(" (active)") : "";
          const keyStatus = process.env[p.keyEnv] ? C.green("✓ KEY") : C.red("✗ " + p.keyEnv);
          console.log(`  ${C.bold(p.name)}${tag}  ${keyStatus}  ${C.dim(p.hostname)}`);
          console.log(`    Default model: ${C.cyan(p.defaultModel)}`);
        }
        console.log();
      } else {
        const provId = arg1.toLowerCase();
        if (!PROVIDERS[provId]) {
          console.log(C.red(`  Unknown provider: ${arg1}`));
          console.log(C.dim(`  Available: ${Object.keys(PROVIDERS).join(", ")}`));
        } else {
          const prov = PROVIDERS[provId];
          config.model = prov.defaultModel;
          saveConfig(config);
          console.log(`  Switched to ${C.bold(prov.name)}: ${C.cyan(prov.defaultModel)}`);
          const keyOk = process.env[prov.keyEnv];
          if (!keyOk) console.log(C.yellow(`  Warning: ${prov.keyEnv} not set.`));
        }
      }
